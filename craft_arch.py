...
class Attention_regular(nn.Module):
    ...
    Args:
        dim (int): Number of input channels.
        idx (int): 0 for H-Rwin, 1 for V-Rwin.
        split_size (tuple[int, int]): Height and Width of the window.
        dim_out (int | None): Output dimension. Default: None
        num_heads (int): Number of attention heads. Default: 6
        qk_scale (float | None): Override default qk scale if set.
        position_bias (bool): Use dynamic relative position bias. Default: True
    """
    def __init__(self, dim, idx, split_size=[2,4], dim_out=None, num_heads=6, qk_scale=None, position_bias=True):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, qkv, H, W, mask=None, rpi=None, rpe_biases=None):
        q, k, v = qkv[0], qkv[1], qkv[2]
        B, L, C = q.shape
        assert L == H * W, "Input length must match H * W"
        ...
        if self.position_bias:
            pos = self.pos(rpe_biases)  # rpe_biases is now a tensor
        ...
        if mask is not None:
            nW = mask.shape[0]
            attn = attn.view(B, nW, self.num_heads, self.H_sp * self.W_sp, self.H_sp * self.W_sp) + mask.unsqueeze(1).unsqueeze(0)
...
class SRWAB(nn.Module):
    r""" Shift Rectangle Window Attention Block with Adaptive Window Sizes.

    Args:
        dim (int): Number of input channels.
        num_heads (int): Number of attention heads.
        split_sizes (list[tuple[int, int]]): List of (H, W) window sizes, e.g., [[2,4], [4,8], [8,16]].
    ...
    def __init__(self,
        ...
        self.num_scales = len(split_sizes)
        self.shift_size = shift_size
        self.mlp_ratio = mlp_ratio
        self.patch_size = patch_size

        # Normalization and QKV projection
        self.norm1 = norm_layer(dim)
        self.qkv = nn.Linear(dim, dim * 3, bias=qkv_bias)
        self.proj = nn.Linear(dim, dim)
        ...
        # Attention modules for H-Rwin (idx=0)
        ...
        # Attention modules for V-Rwin (idx=1)
    ...
    def calculate_mask_for_split(self, split_size, H, W, device):
        img_mask_0 = torch.zeros((1, H, W, 1), device=device)
        img_mask_1 = torch.zeros((1, H, W, 1), device=device)

        h_slices_0 = (
            slice(0, -split_size[0]),
            slice(-split_size[0], -self.shift_size[0]),
            slice(-self.shift_size[0], None)
        )
        w_slices_0 = (
            slice(0, -split_size[1]),
            slice(-split_size[1], -self.shift_size[1]),
            slice(-self.shift_size[1], None)
        )
        ...
        img_mask_0 = img_mask_0.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, split_size[0], split_size[1], 1)
        mask_windows_0 = img_mask_0.view(-1, split_size[0] * split_size[1])
        attn_mask_0 = mask_windows_0.unsqueeze(1) - mask_windows_0.unsqueeze(2)
        attn_mask_0 = attn_mask_0.masked_fill(attn_mask_0 != 0, float(-100.0)).masked_fill(attn_mask_0 == 0, float(0.0))

        img_mask_1 = img_mask_1.view(1, H // split_size[1], split_size[1], W // split_size[0], split_size[0], 1)
        img_mask_1 = img_mask_1.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, split_size[0], split_size[1], 1)
    ...
    def forward(self, x, x_size, params):
        ...
        x_norm = x.view(b, h, w, c).permute(0, 3, 1, 2)
        scale_weights = self.scale_predictor(x_norm).view(b, self.num_scales)
        ...
        if self.shift_size[0] > 0 or self.shift_size[1] > 0:
            ...
            x1_scales = []
            x2_scales = []
            for i, (attn_h, attn_v, split_size) in enumerate(zip(self.attns_h, self.attns_v, self.split_sizes)):
                mask_0, mask_1 = self.calculate_mask_for_split(split_size, h, w, device=x.device)

                # H-Rwin attention
                x1_shift = attn_h(
                    qkv_0, h, w,
                    mask=mask_0,
                    rpi=params['rpi_sa_h'][i],  # Select corresponding index
                    rpe_biases=params['biases_h'][i]  # Select corresponding biases
                )
                x1 = torch.roll(x1_shift, shifts=(self.shift_size[0], self.shift_size[1]), dims=(1, 2))
                x1_scales.append(x1)
                ...
                x2_shift = attn_v(
                    qkv_1, h, w,
                    mask=mask_1,
                    rpi=params['rpi_sa_v'][i],
                    rpe_biases=params['biases_v'][i]
                )
                x2 = torch.roll(x2_shift, shifts=(self.shift_size[1], self.shift_size[0]), dims=(1, 2))
                x2_scales.append(x2)

            x1 = sum(scale_weights[:, i].view(b, 1, 1, 1) * x1_scales[i] for i in range(self.num_scales))
            x2 = sum(scale_weights[:, i].view(b, 1, 1, 1) * x2_scales[i] for i in range(self.num_scales))
            attened_x = torch.cat([x1, x2], dim=-1)

        else:
            x1_scales = [
                attn(qkv[:, :, :, :c//2], h, w, rpi=params['rpi_sa_h'][i], rpe_biases=params['biases_h'][i])
                for i, attn in enumerate(self.attns_h)
            ]
            x2_scales = [
                attn(qkv[:, :, :, c//2:], h, w, rpi=params['rpi_sa_v'][i], rpe_biases=params['biases_v'][i])
                for i, attn in enumerate(self.attns_v)
            ]
