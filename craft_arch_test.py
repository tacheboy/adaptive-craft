...
def img2windows(img, H_sp, W_sp):
    ...
    """
    B, C, H, W = img.shape
    pad_h = 0 if H % H_sp == 0 else H_sp - (H % H_sp)
    pad_w = 0 if W % W_sp == 0 else W_sp - (W % W_sp)
    if pad_h > 0 or pad_w > 0:
        img = F.pad(img, (0, pad_w, 0, pad_h), mode='constant', value=0)
    H_padded = H + pad_h
    W_padded = W + pad_w
    img_reshape = img.view(B, C, H_padded // H_sp, H_sp, W_padded // W_sp, W_sp)
...
def windows2img(img_splits_hw, H_sp, W_sp, H, W):
    ...
    """
    B_prime, N, C = img_splits_hw.shape
    pad_h = 0 if H % H_sp == 0 else H_sp - (H % H_sp)
    pad_w = 0 if W % W_sp == 0 else W_sp - (W % W_sp)
    H_padded = H + pad_h
    W_padded = W + pad_w
    num_windows_h = H_padded // H_sp
    num_windows_w = W_padded // W_sp
    B = B_prime // (num_windows_h * num_windows_w)
    img = img_splits_hw.view(B, num_windows_h, num_windows_w, H_sp, W_sp, C)
    img = img.permute(0, 5, 1, 3, 2, 4).contiguous().view(B, C, H_padded, W_padded)
    img = img[:, :, :H, :W]
    return img
...
class Attention_regular(nn.Module):
    """Regular Rectangle-Window Attention with Dynamic Window Sizes."""

    def __init__(self, dim, idx, split_size=[4,8], num_heads=6, dim_out=None, qk_scale=None, position_bias=True):
        super().__init__()
        self.dim = dim
        self.dim_out = dim_out or dim
        self.split_size = split_size
        ...
        self.H_sp, self.W_sp = (split_size[0], split_size[1]) if idx == 0 else (split_size[1], split_size[0])
        self.pos = DynamicPosBias(self.dim // 4, self.num_heads, residual=False)
        self.softmax = nn.Softmax(dim=-1)

    def im2win(self, x, H, W):
        B, N, C = x.shape
        assert N == H * W, "Input length must match H * W"
        x = x.view(B, H, W, C).permute(0, 3, 1, 2)
        x = img2windows(x, self.H_sp, self.W_sp)
        x = x.reshape(-1, self.H_sp * self.W_sp, self.num_heads, C // self.num_heads).permute(0, 2, 1, 3)
        return x
    ...
    def forward(self, qkv, H, W, mask=None, rpi=None, rpe_biases=None):
        ...
        pad_h = 0 if H % self.H_sp == 0 else self.H_sp - (H % self.H_sp)
        pad_w = 0 if W % self.W_sp == 0 else self.W_sp - (W % W_sp)
        H_padded = H + pad_h
        W_padded = W + pad_w
        nW = (H_padded // self.H_sp) * (W_padded // self.W_sp)
        ...
        if self.position_bias and rpi is not None and rpe_biases is not None:
            pos = self.pos(rpe_biases)
            expected_positions = self.H_sp * self.W_sp * self.H_sp * self.W_sp
            if pos.shape[0] >= expected_positions:
                relative_position_bias = pos[rpi.view(-1)[:expected_positions]].view(
                    self.H_sp * self.W_sp, self.H_sp * self.W_sp, -1)
                relative_position_bias = relative_position_bias.permute(2, 0, 1).contiguous()
                attn = attn + relative_position_bias.unsqueeze(0)

        if mask is not None:
            attn = attn.view(B, nW, self.num_heads, self.H_sp * self.W_sp, self.H_sp * self.W_sp)
            attn = attn + mask.unsqueeze(1).unsqueeze(0)
            attn = attn.view(-1, self.num_heads, self.H_sp * self.W_sp, self.H_sp * self.W_sp)
...
class SRWAB(nn.Module):
    """Shift Rectangle Window Attention Block with Learnable Window Sizes."""

    def __init__(self,
                 dim,
                 num_heads,
                 patch_size=64,
                 split_sizes=[[4,8], [8,16], [12,24]],
                 shift_size=(0,0),
                 mlp_ratio=2.,
                 qkv_bias=True,
                 qk_scale=None,
        ...
        self.num_sizes = len(split_sizes)
        self.shift_size = shift_size
        self.mlp_ratio = mlp_ratio
        self.patch_size = patch_size
        ...
        self.window_size_predictor = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(64, self.num_sizes, kernel_size=1),
            nn.Softmax(dim=1)
        )

        self.attns_h = nn.ModuleList([
            Attention_regular(
                dim=dim//2,
                idx=0,
                split_size=size,
        ...
        self.attns_v = nn.ModuleList([
            Attention_regular(
                dim=dim//2,
                idx=1,
                split_size=[size[1], size[0]],
                num_heads=num_heads//2,
                dim_out=dim//2,
                qk_scale=qk_scale,
                position_bias=True
    ...
    def calculate_mask_for_split(self, split_size, H, W, device):
        H_sp, W_sp = split_size
        pad_h = 0 if H % H_sp == 0 else H_sp - (H % H_sp)
        pad_w = 0 if W % W_sp == 0 else W_sp - (W % W_sp)
        H_padded = H + pad_h
        W_padded = W + pad_w

        img_mask_0 = torch.zeros((1, H_padded, W_padded, 1), device=device)
        img_mask_1 = torch.zeros((1, H_padded, W_padded, 1), device=device)

        shift_h = min(self.shift_size[0], H_padded - H_sp) if self.shift_size[0] > 0 else 0
        shift_w = min(self.shift_size[1], W_padded - W_sp) if self.shift_size[1] > 0 else 0

        h_slices_0 = (
            slice(0, -H_sp),
            slice(-H_sp, -shift_h) if shift_h > 0 else slice(-H_sp, None),
            slice(-shift_h, None) if shift_h > 0 else slice(0, None)
        )
        w_slices_0 = (
            slice(0, -W_sp),
            slice(-W_sp, -shift_w) if shift_w > 0 else slice(-W_sp, None),
            slice(-shift_w, None) if shift_w > 0 else slice(0, None)
        )
        h_slices_1 = (
            slice(0, -W_sp),
            slice(-W_sp, -shift_w) if shift_w > 0 else slice(-W_sp, None),
        ...
        # Horizontal mask (H-Rwin)
        img_mask_0 = img_mask_0.view(1, H_padded // H_sp, H_sp, W_padded // W_sp, W_sp, 1)
        img_mask_0 = img_mask_0.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, H_sp, W_sp, 1)
        mask_windows_0 = img_mask_0.view(-1, H_sp * W_sp)
        attn_mask_0 = mask_windows_0.unsqueeze(1) - mask_windows_0.unsqueeze(2)
        attn_mask_0 = attn_mask_0.masked_fill(attn_mask_0 != 0, float(-100.0)).masked_fill(attn_mask_0 == 0, float(0.0))

        # Vertical mask (V-Rwin)
        img_mask_1 = img_mask_1.view(1, H_padded // W_sp, W_sp, W_padded // H_sp, H_sp, 1)
        img_mask_1 = img_mask_1.permute(0, 1, 3, 2, 4, 5).contiguous().view(-1, W_sp, H_sp, 1)
        mask_windows_1 = img_mask_1.view(-1, W_sp * H_sp)
        attn_mask_1 = mask_windows_1.unsqueeze(1) - mask_windows_1.unsqueeze(2)
        attn_mask_1 = attn_mask_1.masked_fill(attn_mask_1 != 0, float(-100.0)).masked_fill(attn_mask_1 == 0, float(0.0))
    ...
    def forward(self, x, x_size, params):
        ...
        window_probs = self.window_size_predictor(x_2d)
        window_indices = window_probs.argmax(dim=1)
        ...
        if self.shift_size[0] > 0 or self.shift_size[1] > 0:
            ...
            mode_size_idx = torch.mode(window_indices.view(b, -1), dim=1).values[0].item()
            selected_size = self.split_sizes[mode_size_idx]
            attn_h = self.attns_h[mode_size_idx]
            attn_v = self.attns_v[mode_size_idx]
            mask_0, mask_1 = self.calculate_mask_for_split(selected_size, h, w, x.device)

            x1_shift = attn_h(
                qkv_0, h, w,
                mask=mask_0,
                rpi=params['rpi_sa_h'][mode_size_idx],
                rpe_biases=params['biases_h'][mode_size_idx]
            )
            x1 = torch.roll(x1_shift, shifts=(self.shift_size[0], self.shift_size[1]), dims=(1, 2))
            x2_shift = attn_v(
                qkv_1, h, w,
                mask=mask_1,
                rpi=params['rpi_sa_v'][mode_size_idx],
                rpe_biases=params['biases_v'][mode_size_idx]
            )
            x2 = torch.roll(x2_shift, shifts=(self.shift_size[1], self.shift_size[0]), dims=(1, 2))
        ...
        attened_x = attened_x.view(b, -1, c).contiguous()
        lcm = self.get_v(v).permute(0, 2, 3, 1).contiguous().view(b, -1, c)
        attened_x = attened_x + lcm
        attened_x = self.proj(attened_x)
...
#         # High-frequency enhancement branch with multi-scale dilated convolutions
#         self.dilated_conv1 = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=3, padding=1, dilation=1)
#         self.dilated_conv2 = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=3, padding=2, dilation=2)
#         self.dilated_conv3 = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=3, padding=3, dilation=3)
#         self.fc = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=1)

#         # Local feature extraction branch
#         self.conv = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=3, padding=1)
...
#         # High-frequency enhancement branch with multi-scale dilated convolutions
#         high_input = x[:, self.mid_dim:, :, :]
#         hfe1 = self.dilated_conv1(high_input)  # Dilation=1 (standard 3x3 conv)
#         hfe2 = self.dilated_conv2(high_input)  # Dilation=2 (larger receptive field)
#         hfe3 = self.dilated_conv3(high_input)  # Dilation=3 (even larger receptive field)
#         hfe = hfe1 + hfe2 + hfe3  # Sum to combine multi-scale features
#         hfe = self.act(self.fc(hfe))  # Reduce with 1x1 conv and activate

#         # Concatenate and apply final convolution with residual connection
...
class HFERB(nn.Module):
    def __init__(self, dim):
        ...
        # High-frequency enhancement branch with edge-aware convolution
        self.edge_conv = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=3, padding=1)
        self.fc = nn.Conv2d(self.mid_dim, self.mid_dim, kernel_size=1)

        # Initialize edge_conv with Laplacian filters for edge awareness
        laplacian_kernel = torch.tensor([[0, 1, 0], [1, -4, 1], [0, 1, 0]], dtype=torch.float32).view(1, 1, 3, 3)
        with torch.no_grad():
            self.edge_conv.weight[:, :, :, :] = 0
            for i in range(self.mid_dim):
                self.edge_conv.weight[i, i, :, :] = laplacian_kernel[0, 0, :, :]
    ...
    def forward(self, x):
        ...
        # High-frequency enhancement branch with edge-aware convolution
        edge_features = self.edge_conv(x[:, self.mid_dim:, :, :])
        hfe = self.act(self.fc(edge_features))
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads, bias=True, train_size=(1, 3, 48, 48), base_size=(72, 72)):
        """
        Initialize the Attention module for MDTA in HFB.
        
        Args:
            dim (int): Number of input channels.
            num_heads (int): Number of attention heads.
            bias (bool): Whether to include bias in convolutions.
            train_size (tuple): Training input size (B, C, H, W).
            base_size (tuple): Base size for scaling, default (72, 72).
        """
        super(Attention, self).__init__()
        self.num_heads = num_heads
        self.dim = dim
        ...
        # 1x1 convolutions for Q, KV projections (no depth-wise convolutions)
        self.q = nn.Conv2d(dim, dim, kernel_size=1, bias=bias)
        self.kv = nn.Conv2d(dim, dim * 2, kernel_size=1, bias=bias)
    ...
    def _forward(self, q, kv):
        ...
        Args:
            q (Tensor): Query tensor of shape (B, dim, H, W).
            kv (Tensor): Key-Value tensor of shape (B, dim*2, H, W).
        
        Returns:
            Tensor: Attention output of shape (B, dim, H, W) before rearrangement.
        """
        ...
        out = attn @ v  # (B, head, c, h*w) @ (B, head, h*w, c) -> (B, head, c, h*w)
    ...
    def forward(self, low, high):
        ...
        Args:
            low (Tensor): Low-resolution input features (B, dim, H, W).
            high (Tensor): High-resolution input features (B, dim, H, W).
        
        Returns:
            Tensor: Output features (B, dim, H, W).
        """
        # Store spatial dimensions
        self.h, self.w = low.shape[2:]
        
        # Compute Q from high, KV from low using 1x1 convolutions only
        q = self.q(high)    # (B, dim, H, W)
        kv = self.kv(low)   # (B, dim*2, H, W)
        
        # Perform attention computation
        out = self._forward(q, kv)  # (B, head, c, h*w)
        
        # Rearrange back to spatial dimensions
        out = rearrange(out, 'b head c (h w) -> b (head c) h w', 
                       head=self.num_heads, h=self.h, w=self.w)  # (B, dim, H, W)
...
#         self.attn = Attention(dim=dim, num_heads=num_heads, bias=True)
...
class HFB(nn.Module):
    def __init__(self, dim, num_heads, ffn_expansion_factor, bias, LayerNorm_type):
        ...
        self.norm2 = LayerNorm(dim, LayerNorm_type)
        self.ffn = FeedForward(dim, ffn_expansion_factor, bias)
        self.spatial_attn = SpatialAttention(dim)  # Added spatial attention
        self.dim = dim

    def forward(self, low, high):
        self.h, self.w = low.shape[2:]
        # Compute attention and fuse with low-frequency features
        attn_output = self.attn(self.norm1(low), high)
        fused = low + attn_output
        # Apply spatial attention to refine fused features
        fused = self.spatial_attn(fused)
        # Proceed with normalization and FFN
        x = fused + self.ffn(self.norm2(fused))
        return x
...
class CRFB(nn.Module):
    ...
    Args:
        ...
        split_sizes (list[tuple[int, int]]): List of window sizes for adaptive attention, e.g., [[2,4], [4,8], [8,16]].
    """
    ...
    def __init__(self,
                 ...
                 num_heads,
                 patch_size=64,
                 split_sizes=[[2,4], [4,8], [8,16]],  # New parameter for adaptive window sizes
                 mlp_ratio=2.,
                 qkv_bias=True,
                 qk_scale=None,
                 norm_layer=nn.LayerNorm
        ...
        # Shift Rectangle window attention blocks with adaptive window sizes
        self.srwa_blocks = nn.ModuleList([
            SRWAB(
                dim=dim,
                num_heads=num_heads,
                patch_size=patch_size,
                split_sizes=split_sizes,  # Pass the list of window sizes
                shift_size=[0,0] if (i % 2 == 0) else [split_sizes[0][0]//2, split_sizes[0][1]//2],  # Use first split_size for shifting
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
        ...
        # High frequency enhancement residual blocks (unchanged)
        self.hfer_blocks = nn.ModuleList([
            HFERB(dim) for _ in range(depth)
        ])

        # Hybrid fusion blocks (unchanged)
        self.hf_blocks = nn.ModuleList([
            HFB(
                dim=dim,
                num_heads=num_heads,
...
class CRAFT(nn.Module):
    ...
    Args:
        ...
        qk_scale (float): Override default qk scale of head_dim ** -0.5 if set. Default: None
        norm_layer (nn.Module): Normalization layer. Default: nn.LayerNorm
        upscale (int): Upscale factor. 2/3/4
        img_range (float): Image range. 1. or 255.
        upsampler (str): Upsampler type. Default: ''
        resi_connection (str): Convolutional block before residual connection. '1conv'/'identity'
        patch_size (int): Patch size for attention blocks. Default: 64
        split_sizes (list[tuple[int, int]]): List of window sizes for adaptive attention. Default: [[2,4], [4,8], [8,16]]
    """
    ...
    def __init__(self,
                 ...
                 depths=(6, 6, 6, 6),
                 num_heads=(6, 6, 6, 6),
                 split_sizes=[[2,4], [4,8], [8,16]],  # Updated for adaptive window sizes
                 mlp_ratio=2.,
                 qkv_bias=True,
                 qk_scale=None,
                 norm_layer=nn.LayerNorm,
        ...
        # Relative position indices for all split sizes
        self.split_sizes = split_sizes
        self.calculate_rpi_v_sa()
        ...
        # Build Residual Cross-Refinement Fusion Groups
        self.layers = nn.ModuleList()
        for i_layer in range(self.num_layers):
            layer = RCRFG(
                dim=embed_dim,
                ...
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
                split_sizes=split_sizes,  # Pass adaptive window sizes
                norm_layer=norm_layer
            )
            self.layers.append(layer)
    ...
    def calculate_rpi_v_sa(self):
        # Compute relative position indices and biases for each split_size
        self.relative_position_index_h = []
        self.relative_position_index_v = []
        self.biases_h = []
        self.biases_v = []
        ...
        for split_size in self.split_sizes:
            ...
            # H-Rwin relative position index
            coords_h = torch.arange(H_sp)
            coords_w = torch.arange(W_sp)
            coords = torch.stack(torch.meshgrid([coords_h, coords_w]))
            coords_flatten = torch.flatten(coords, 1)
            ...
            # V-Rwin biases (swap H_sp and W_sp)
            H_sp, W_sp = W_sp, H_sp
            position_bias_h = torch.arange(1 - H_sp, H_sp)
            position_bias_w = torch.arange(1 - W_sp, W_sp)
            biases_v = torch.stack(torch.meshgrid([position_bias_h, position_bias_w]))
            ...
            # V-Rwin relative position index
            coords_h = torch.arange(H_sp)
            coords_w = torch.arange(W_sp)
            coords = torch.stack(torch.meshgrid([coords_h, coords_w]))
            coords_flatten = torch.flatten(coords, 1)
            ...
            # Register buffers
            self.relative_position_index_h.append(relative_position_index_h)
            self.relative_position_index_v.append(relative_position_index_v)
            self.biases_h.append(biases_h)
            self.biases_v.append(biases_v)

        # Convert lists to tuples for buffer registration
        for i in range(len(self.split_sizes)):
            self.register_buffer(f'relative_position_index_h_{i}', self.relative_position_index_h[i])
            self.register_buffer(f'relative_position_index_v_{i}', self.relative_position_index_v[i])
            self.register_buffer(f'biases_h_{i}', self.biases_h[i])
            self.register_buffer(f'biases_v_{i}', self.biases_v[i])
    ...
    def forward_features(self, x):
        x_size = (x.shape[2], x.shape[3])
        # Prepare params for all split sizes
        params = {
            'attn_mask': (None, None),
            'rpi_sa_h': [getattr(self, f'relative_position_index_h_{i}') for i in range(len(self.split_sizes))],
            'rpi_sa_v': [getattr(self, f'relative_position_index_v_{i}') for i in range(len(self.split_sizes))],
            'biases_h': [getattr(self, f'biases_h_{i}') for i in range(len(self.split_sizes))],
            'biases_v': [getattr(self, f'biases_v_{i}') for i in range(len(self.split_sizes))]
        }
