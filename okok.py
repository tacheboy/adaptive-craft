...
class CARAFEUpsample(nn.Module):
    """CARAFE: Content-Aware ReAssembly of Features for Upsampling.

    Args:
        scale (int): Upscale factor (e.g., 2, 3, 4).
        num_feat (int): Number of input feature channels (embed_dim).
        num_out_ch (int): Number of output channels (e.g., 3 for RGB).
        kernel_size (int): Size of the reassembly kernel (default: 5).
        encoder_channels (int): Channels for kernel prediction (default: 64).
    """

    def __init__(self, scale, num_feat, num_out_ch, kernel_size=5, encoder_channels=64):
        ...
        self.kernel_encoder = nn.Sequential(
            nn.Conv2d(num_feat, encoder_channels, kernel_size=3, stride=1, padding=1),
            nn.ReLU(inplace=True),
            nn.Conv2d(encoder_channels, scale * scale * kernel_size * kernel_size, kernel_size=1)
        )

        # Feature projection to output channels
        self.feature_proj = nn.Conv2d(num_feat, num_out_ch, kernel_size=1)

    def forward(self, x):
        """Forward pass.

        Args:
            x (torch.Tensor): Input tensor of shape (B, num_feat, H, W).

        Returns:
            torch.Tensor: Upsampled tensor of shape (B, num_out_ch, H*scale, W*scale).
        """
        B, C, H, W = x.shape

        # Predict reassembly kernels
        kernels = self.kernel_encoder(x)  # [B, scale*scale*kernel_size*kernel_size, H, W]
        kernels = kernels.view(B, self.scale * self.scale, self.kernel_size * self.kernel_size, H, W)
        kernels = F.softmax(kernels, dim=2)  # Normalize over kernel_size dimension

        # Project features to output channels
        x = self.feature_proj(x)  # [B, num_out_ch, H, W]

        # Upsample using CARAFE
        out_H, out_W = H * self.scale, W * self.scale
        output = torch.zeros(B, self.num_out_ch, out_H, out_W, device=x.device)
        ...
        for i in range(self.scale):
            for j in range(self.scale):
                # Extract kernel for this upsampling position
                kernel = kernels[:, i * self.scale + j]  # [B, kernel_size*kernel_size, H, W]

                # Compute output for this sub-pixel position
                for b in range(B):
                    for c in range(self.num_out_ch):
                        # Gather features using the kernel
                        gathered = F.unfold(
                            x_padded[b:b+1, c:c+1],
                            kernel_size=self.kernel_size,
                            stride=1,
                            padding=0
                        )  # [1, kernel_size*kernel_size, H*W]
                        gathered = gathered.view(1, self.kernel_size * self.kernel_size, H, W)
                        weighted = gathered * kernel[b:b+1]  # [1, kernel_size*kernel_size, H, W]
                        reassembled = weighted.sum(dim=1)  # [1, H, W]
                        output[b, c, i::self.scale, j::self.scale] = reassembled.view(H, W)
...
class CRAFT(nn.Module):
    ...
    def __init__(self,
                 ...
                 upscale=4,
                 img_range=1.,
                 upsampler='carafe',  # Indicate CARAFE upsampling
                 resi_connection='1conv',
                 hooks=False,
                 **kwargs):
        super(CRAFT, self).__init__()
